 ...
 function parse($adverts) : array{
    ...
    foreach ($adverts as $adverts) {
        ...
        if ($category === 'rent') {
            ...
            $obj_advert= new SellAdvert(
                type: $type,
                rooms: $price,
                price: $rooms
            );
        } else {
            exit ("К сожалению на нашем сайте еще нет такой функции!");
        }
        $objArray[] = $obj_advert;
    }
return $obj_arr;
...
foreach ($adv_obj as $adv) {
    $created_add = create_adds($adv);
    echo $created_add . PHP_EOL;
}
